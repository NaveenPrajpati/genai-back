...
- **Code root:** [`app/routers/rag.py`](app/routers/rag.py) (HTTP) + [`app/services/`](app/services/) (pipeline).
...
| **Jina** | Hosted cross-encoder reranker (`jina-reranker-v2-base-multilingual`) | [`services/retrieval.py`](app/services/retrieval.py) |
| **BM25** | Sparse keyword encoder (`pinecone_text`, MS-MARCO default) | [`services/retrieval.py`](app/services/retrieval.py) |
| **Redis** | Semantic cache (embedding-keyed, 24 h TTL) | [`services/cache.py`](app/services/cache.py) |
| **Supabase** | Relational store: ingestion logs, chats, messages | [`services/storage.py`](app/services/storage.py) |
...
Written by [`retrieval.hybrid_add_texts`](app/services/retrieval.py). Each vector carries
...
### Step 1 — Ingestion / loading · [`services/ingestion.py`](app/services/ingestion.py)
...
### Step 2 — Chunking · [`utils/chunking.py`](app/utils/chunking.py)
...
### Step 3 — Embedding (hybrid) · [`services/retrieval.py`](app/services/retrieval.py)
...
### Step 4 — Retrieval · [`services/retrieval.py`](app/services/retrieval.py)
...
### Step 5 — Reranking & ordering · [`services/retrieval.py`](app/services/retrieval.py)
...
### Step 6 — Context prep + citations · [`services/generation.py`](app/services/generation.py)
...
### Step 7 — Grounding enforcement · [`services/grounding.py`](app/services/grounding.py)
Three layers so the system **declines rather than confabulates**:
1. **Answerability gate** (`is_answerable`) — a cheap `gpt-4o-mini` structured check *before*
...
### Persistence · [`services/storage.py`](app/services/storage.py)
...
### Evaluation (online, optional) · [`services/evaluation.py`](app/services/evaluation.py)
...
{ "answer": "Q3 revenue was $4.2M [2].",
  "sources": [ {"citation":2,"chunk_text":"...","source":"financials_q3.pdf",
                "page_number":3,"confidence_score":0.94,"doc_id":"..."} ],
  "grounded": true,
  "evaluation": { "retrieval_precision":0.8, "recall_score":1.0, "hallucination_rate":0.0 } }
```
...
- **Every** write stamps `user_id`; **every** read/cache/delete is scoped to the caller.
  Empty `ingestions` means "all of *my* documents", never everyone's.
...
## 10. Offline evaluation & CI · [`app/evals/rag_eval.py`](app/evals/rag_eval.py)
...
```bash
python -m app.evals.rag_eval --self-test                                   # offline, no keys
python -m app.evals.rag_eval --data app/evals/datasets/rag_golden.jsonl    # real eval
python -m app.evals.rag_eval --data ... --gate                             # enforce thresholds
```
...
| Loading (Step 1) | [`app/services/ingestion.py`](app/services/ingestion.py) |
| Chunking (Step 2) | [`app/utils/chunking.py`](app/utils/chunking.py) |
| Embed / retrieve / rerank (Steps 3–5) | [`app/services/retrieval.py`](app/services/retrieval.py) |
| Background ingest worker | [`app/services/ingestion_worker.py`](app/services/ingestion_worker.py) |
| Context + citations (Step 6) | [`app/services/generation.py`](app/services/generation.py) |
| Grounding gate / sentinel | [`app/services/grounding.py`](app/services/grounding.py) |
| Semantic cache | [`app/services/cache.py`](app/services/cache.py) |
| Online judges | [`app/services/evaluation.py`](app/services/evaluation.py) |
| Persistence | [`app/services/storage.py`](app/services/storage.py) |
...
| Offline eval + golden set | [`app/evals/rag_eval.py`](app/evals/rag_eval.py), [`app/evals/datasets/rag_golden.jsonl`](app/evals/datasets/rag_golden.jsonl) |
